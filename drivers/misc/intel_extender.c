...
vm_fault_t intel_extender_el0_fault(struct vm_fault *vmf)
{
	 ...
	 * accesses it faults again and goes again the steps up to it was
	...
	if (reclaimed_window) {
		...
		if (mmget_not_zero(mm)) {
			another_task_vma = find_vma_intersection(mm, addr,
								 addr + 1);
			if (another_task_vma) {
				dev_dbg(extender->dev,
					"(el0) zap_vma_ptes(addr %016lx)\n",
					addr);
				zap_vma_ptes(another_task_vma, addr,
					     PAGE_SIZE);
			}
			mmput(mm);
		} else {
			     ...
			     struct pt_regs *regs)
{
	...
	if (is_mapped(faulting_addr, 1, false)) {
		...
		trace_extender_fault_handler_exit(stringify_el(faulting_addr),
			"resolved (fast-path) paging request at VA",
			faulting_addr);
		return 0;
	}
	...
	if (extender_page_range(mapping_addr, mapping_addr + first_in->size,
				 ...
				 first_in->caller)) {
		unsigned long end = (unsigned long)mapping_addr +
				    first_in->size;
...
static void run_some_diagnostics(void)
{
	...
	dev_dbg(extender->dev, "PCI_IO_START %lx PCI_IO_END %lx PCI_IO_SIZE %x\n",
		PCI_IO_START, PCI_IO_END, (unsigned int)PCI_IO_SIZE);
...
static int intel_extender_probe(struct platform_device *pdev)
{
	...
	BUG_ON((unsigned long)extender->el1.extender_start +
	       extender->el1.extender_size > EXTENDER_END);
	...
	for (i = 0; i < pdev->num_resources; i += 2) {
		 ...
		 *
		 * Let us go step by step what we do below. We get the count
		 * order (fls - find last most-significant bit set) after
		 * rounding up to power of 2 and return if it is in the
		 * range [PAGE_SHIFT,
		 * get_count_order_long(EXTENDER_WINDOW_MAX_SIZE))]. If not
		 * we return either lower or upper boundary upon what
		 * the boundery is crossed, eg. if the size (as from the
		 * order) is less than a page size (page size has PAGE_SHIFT
		 * order) than we take PAGE_SHIFT and not the order
		 * calculated.
		 *
		 * However as windowed_slave size is not in our control
		 * (it is Intel's extender IP) and we take it as-is we
		 * can only warn/bug on finding the misconfiguration.
		 */
		window_expected_size = 1ul << clamp_t(int,
				get_count_order_long(win->size),
				PAGE_SHIFT,
				get_count_order_long(EXTENDER_WINDOW_MAX_SIZE));

		dev_dbg(extender->dev, "window expected size %lx get_count_order_long(win->size %lx) %d fls(extender->win->size) %d\n",
